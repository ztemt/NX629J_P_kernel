...
static void  i2c_check_func(struct work_struct *worker)
{
	...
	if(g_power_stat != 0)
		...
		LOG("cs_press power is down,no need to check i2c!\n");
	...
	size_t count)
{
	...
	set_press_threshold(thd_val, thd_val*80/100, 0xff);
	...
	struct device_attribute *attr, char *buf)
{
	...
	get_press_threshold(&thd_down_val, &thd_up_val, 0xff);
