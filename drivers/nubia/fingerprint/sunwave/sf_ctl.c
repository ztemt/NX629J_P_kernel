                                   ...
                                   unsigned long event, void *data)
{
    ...
    if (event != MSM_DRM_EARLY_EVENT_BLANK /*FB_EVENT_BLANK FB_EARLY_EVENT_BLANK */) {
        return 0;
    }
...
static int sf_probe(sf_device_t *dev)
{
	...
	if (id_gpio) {
		xprintk(KERN_INFO, "id_gpio = %d. It's fingerprint chip of vendor : sunwave\n", id_gpio);
		gpio_free(NUBIA_SENSORS_FP_ID_GPIO);
		//auto_dev->power_on(false);
		//sf_ctl_dev.free_gpio(&sf_ctl_dev);
	}
	else {
		xprintk(KERN_ERR, "id_gpio = %d. It's fingerprint chip of vendor : silead, now driver will exit\n", id_gpio);
		gpio_free(NUBIA_SENSORS_FP_ID_GPIO);
		auto_dev->power_on(false);
		sf_ctl_dev.free_gpio(&sf_ctl_dev);
		return -ENODEV;
...
static int __init sf_ctl_driver_init(void)
{
	...
	if (err == (-ENODEV)) {
	   xprintk(KERN_ERR, "%s, Failed to register platform driver. err == ENODEV\n", __FUNCTION__);
	   return -ENODEV;
   }
