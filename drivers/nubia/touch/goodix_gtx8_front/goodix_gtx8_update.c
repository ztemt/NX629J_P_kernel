...
#include "goodix_front_default_fw.h"
/* COMMON PART - START */
#define TS_DEFAULT_FIRMWARE			"goodix_front_firmware.bin"
...
atomic_t fw_front_update_mode = ATOMIC_INIT(1);
 ...
 */
static int goodix_check_update(struct goodix_front_ts_device *dev,
 ...
 *		   < 0 failed
 */
static int goodix_reg_write_confirm(struct goodix_front_ts_device *dev,
...
static inline int goodix_reg_write(struct goodix_front_ts_device *dev,
		unsigned int addr, unsigned char *data, unsigned int len)
{
	return dev->hw_ops->write_trans(dev, addr, data, len);
}

static inline int goodix_reg_read(struct goodix_front_ts_device *dev,
		unsigned int addr, unsigned char *data, unsigned int len)
{
...
static int goodix_load_mask(struct goodix_front_ts_device *ts_dev)
{
 ...
 * return 0 ok, <0 error
 */
static int goodix_load_isp(struct goodix_front_ts_device *ts_dev,
			   struct firmware_data *fw_data)
{
...
{
	struct goodix_front_ts_device *ts_dev = fwu_ctrl->ts_dev;
	u8 reg_val[4] = { 0x00 };
...
/*static int goodix_read_back_check(struct goodix_front_ts_device *ts_dev, u16 addr,
 ...
 * return：0 ok, <0 error
 */
static int goodix_send_fw_packet(struct goodix_front_ts_device *dev, u8 type,
				 u8 *pkt, u32 len)
{
	u8 reg_val[4];
	int r, i;
 ...
 * return: 0 ok, < 0 error
 */
static int goodix_flash_subsystem(struct goodix_front_ts_device *dev,
 ...
 * return: 0 ok, < 0 error
 */
static int goodix_flash_firmware(struct goodix_front_ts_device *dev,
				struct firmware_data *fw_data)
{
 ...
 * return: 0 ok, < 0 error
 */
static int goodix_update_finish(struct goodix_front_ts_device *ts_dev,
...
/**
 * goodix_front_fw_update_proc - firmware update process, the entry of
 *  firmware update flow
 * @fwu_ctrl: firmware control
 * return: 0 ok, < 0 error
 */
int goodix_front_fw_update_proc(struct fw_update_ctrl *fwu_ctrl)
{
...
static int goodix_fw_update_thread(void *data)
{
	...
	if (!fwu_ctrl) {
		...
		goodix_front_unregister_ext_module(&goodix_fwu_module);
		return 0;
	}

	mutex_lock(&fwu_lock);
	if (!fwu_ctrl->fw_from_sysfs) {
		if (atomic_read(&fw_front_update_mode) == 0) {
			...
			temp_firmware->size = sizeof(goodix_front_default_fw);
			temp_firmware->data = goodix_front_default_fw;
			fwu_ctrl->fw_data.firmware = temp_firmware;
		} else if (atomic_read(&fw_front_update_mode) == 1) {
	...
	fwu_ctrl->allow_reset = false;
	goodix_front_ts_blocking_notify(NOTIFY_FWUPDATE_START, NULL);

	/* ready to update */
	goodix_front_fw_update_proc(fwu_ctrl);

	goodix_front_ts_blocking_notify(NOTIFY_FWUPDATE_END, NULL);
	fwu_ctrl->allow_reset = true;
	...
	if (!fwu_ctrl->core_data->cfg_group_parsed)
		goodix_front_cfg_bin_proc(fwu_ctrl->core_data);

out:
	goodix_front_unregister_ext_module(&goodix_fwu_module);
	mutex_unlock(&fwu_lock);
	atomic_set(&fw_front_update_mode, 0);
	return 0;
}
		...
		const char *buf, size_t count)
{
	...
	if (r) {
		atomic_set(&fw_front_update_mode, 1);
		if (goodix_front_register_ext_module(&goodix_fwu_module))
			return -EIO;
	}
		...
		struct goodix_ext_module *module)
{
	...
	ktype = goodix_front_get_default_ktype();
...
{
	return goodix_front_register_ext_module(&goodix_fwu_module);
}
