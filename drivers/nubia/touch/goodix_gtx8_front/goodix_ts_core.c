...
//		goodix_ts_gpio_power_on_off(core_data, true);
		//goodix_front_ts_power_on(core_data);
		...
		unsigned long event, void *data)
{
	...
	if (fb_event && fb_event->data && core_data) {
		if (event == MSM_DRM_SWITCH_EARLY_EVENT_BLANK) {
			blank = fb_event->data;
			pr_err("front blank:%u",*blank);
			pr_err("front tp_mode:%u",tp_mode);
			switch (*blank) {
