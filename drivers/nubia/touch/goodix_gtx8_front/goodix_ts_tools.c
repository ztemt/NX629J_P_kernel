...
/*
 * struct goodix_front_tools_dev - goodix tools device struct
 ...
 * @mutex: Protect goodix_front_tools_dev
*/
struct goodix_front_tools_dev {
...
} *goodix_front_tools_dev;


/* read data from i2c asynchronous,
** success return bytes read, else return <= 0
*/
static int async_read(struct goodix_front_tools_dev *dev, void __user *arg)
{
	u8 *databuf = NULL;
	int ret = 0;
	u32 reg_addr, length;
	u8 i2c_msg_head[I2C_MSG_HEAD_LEN];
	struct goodix_front_ts_device *ts_dev = dev->ts_core->ts_dev;
...
static int read_config_data(struct goodix_front_ts_device *ts_dev, void __user *arg)
{
	int ret = 0;
	u32 reg_addr, length;
	u8 i2c_msg_head[I2C_MSG_HEAD_LEN];
...
** success return bytes read, else return <= 0
*/
static int sync_read(struct goodix_front_tools_dev *dev, void __user *arg)
{
	int ret = 0;
	u8 i2c_msg_head[I2C_MSG_HEAD_LEN];
...
*/
static int async_write(struct goodix_front_tools_dev *dev, void __user *arg)
{
	u8 *databuf;
	int ret = 0;
	u32 reg_addr, length;
...
{
	int ret = 0;
	struct goodix_front_tools_dev *dev = filp->private_data;
	...
	switch (cmd & NEGLECT_SIZE_MASK) {
		case GTP_IRQ_ENABLE:
		if (arg == 1) {
			goodix_front_ts_irq_enable(dev->ts_core, true);
			mutex_lock(&dev->mutex);
			dev->ops_mode |= IRQ_FALG;
			mutex_unlock(&dev->mutex);
			ts_info("IRQ enabled");
		} else if (arg == 0) {
			goodix_front_ts_irq_enable(dev->ts_core, false);
			mutex_lock(&dev->mutex);
			dev->ops_mode &= ~IRQ_FALG;
			mutex_unlock(&dev->mutex);
	...
	case GTP_ESD_ENABLE:
		if (arg == 0)
			goodix_front_ts_blocking_notify(NOTIFY_ESD_OFF, NULL);
		else
			goodix_front_ts_blocking_notify(NOTIFY_ESD_ON, NULL);
		break;
	case GTP_DEV_RESET:
		hw_ops->reset(ts_dev);
		break;
...
{
	int ret = 0;
	filp->private_data = goodix_front_tools_dev;
	ts_info("tools open");
	/* Only the first time open device need to register module */
	ret = goodix_front_register_ext_module(&goodix_front_tools_dev->module);
	if (ret) {
...
static int goodix_tools_release(struct inode *inode, struct file *filp)
{
	...
	ret = goodix_front_unregister_ext_module(&goodix_front_tools_dev->module);
	return ret;
}
...
static int goodix_tools_module_irq(struct goodix_ts_core *core_data,
	struct goodix_ext_module *module)
{
	struct goodix_front_tools_dev *dev = module->priv_data;
	struct goodix_front_ts_device *ts_dev = dev->ts_core->ts_dev;
	const struct goodix_ts_hw_ops *hw_ops = ts_dev->hw_ops;
	struct goodix_tools_data *tools_data;
	int r = 0;
...
static int goodix_tools_module_init(struct goodix_ts_core *core_data,
			struct goodix_ext_module *module)
{
	struct goodix_front_tools_dev *tools_dev = module->priv_data;
...
static int __init goodix_tools_init(void)
{
	...
	goodix_front_tools_dev = kzalloc(sizeof(struct goodix_front_tools_dev), GFP_KERNEL);
	if (goodix_front_tools_dev == NULL) {
		ts_err("Memory allco err");
		return -ENOMEM;
	}

	INIT_LIST_HEAD(&goodix_front_tools_dev->head);
	goodix_front_tools_dev->ops_mode = 0;
	goodix_front_tools_dev->ops_mode |= IRQ_FALG;
	init_waitqueue_head(&goodix_front_tools_dev->wq);
	mutex_init(&goodix_front_tools_dev->mutex);
	atomic_set(&goodix_front_tools_dev->t_count, 0);

	goodix_front_tools_dev->module.funcs = &goodix_tools_module_funcs;
	goodix_front_tools_dev->module.name = GOODIX_TOOLS_NAME;
	goodix_front_tools_dev->module.priv_data = goodix_front_tools_dev;
	goodix_front_tools_dev->module.priority = EXTMOD_PRIO_DBGTOOL;
