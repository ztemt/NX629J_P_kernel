...
int g_gesture_flag;
#ifdef GOODIX_TS_GESTURE
extern int g_gesture_enabled;
#endif
...
//add for power on 1v8 morgen
/**
 * goodix_ts_gpio_power_request - Turn on power to the touch device
 * @core_data: pointer to touch core data
 * return: 0 ok, <0 failed
 */
...
/**
 * goodix_ts_gpio_power_on_off - Turn on power to the touch device
 * @core_data: pointer to touch core data
 * return: 0 ok, <0 failed
 */
int goodix_ts_gpio_power_on_off(struct goodix_ts_core *core_data, bool on)
{
	...
	if (on) {
		if (ts_bdata->power_gpio >= 0)
			gpio_direction_output(ts_bdata->power_gpio, 1);
	} else {
		if (ts_bdata->power_gpio >= 0)
			gpio_direction_output(ts_bdata->power_gpio, 0);
	}
...
static int goodix_ts_resume(struct goodix_ts_core *core_data)
{
	...
	if (!g_gesture_enabled && g_gesture_flag) {
		ts_info("start power on");
		//add power on 3.0 and 1.8v
		goodix_ts_gpio_power_on_off(core_data, true);
