...
int b_gesture_enabled;
#endif

extern get_nubia_mode(void);
struct goodix_module goodix_modules;

/*The flag is used to back panel suspend after first startup*/
static int nubia_tp_mode = FRONT_PANEL_MODE;
/**
...
#ifdef GOODIX_TS_BACK_PALM
		if (ts_event->event_type == EVENT_BACK_PALM) {
			/* report PALM */
...
/*add power on 1.8V */
int goodix_ts_back_gpio_power_request(struct goodix_ts_core *core_data, bool reg)
{
	struct goodix_ts_board_data *ts_bdata = board_data(core_data);
	int r;
...
int goodix_ts_back_gpio_power_on_off(struct goodix_ts_core *core_data, bool on)
{
	struct goodix_ts_board_data *ts_bdata = board_data(core_data);
	if(on) {
...
/*static void goodix_nubia_set_rst(struct goodix_ts_core *core_data, bool on)
{
	struct goodix_ts_board_data *ts_bdata =
		board_data(core_data);

	if (on) {
		ts_info("%s: set rst enable\n", __func__);

		if (ts_bdata->reset_gpio >= 0) {
			gpio_set_value(ts_bdata->reset_gpio, 1);
			usleep_range(2000,2000);
		}
	} else {
		ts_info("%s: set rst disable\n", __func__);
		if (ts_bdata->reset_gpio >= 0) {
...
//#ifndef GOODIX_TS_BACK_GESTURE
	/* disable irq */
//	goodix_ts_irq_enable(core_data, false);
//#else
	/* nubia add for power off start */
//	if (!b_gesture_enabled && b_gesture_flag) {
//		ts_info("back start power off");
		//goodix_ts_irq_enable(core_data, false);
		//add power on 3.0 and 1.8v
		//goodix_nubia_set_rst(core_data, 0);
		//goodix_ts_power_off(core_data);
//		goodix_ts_back_gpio_power_on_off(core_data, false);
//		goto out;
//	}
	/* nubia add for power off end */
//#endif
	/* inform external module */
	mutex_lock(&goodix_modules.mutex);
	if (!list_empty(&goodix_modules.head)) {
...
//	if (!b_gesture_enabled && b_gesture_flag) {
//		ts_info("start back power on");
		//add power on 3.0 and 1.8v
//		goodix_ts_back_gpio_power_on_off(core_data, true);
		//goodix_back_ts_power_on(core_data);
		//goodix_nubia_set_rst(core_data, 1);
		//goodix_ts_irq_enable(core_data, true);
//	}
	/* nubia add for power on end */
//	b_gesture_flag = 1;
//#endif
	...
	unsigned long event, void *data)
{
	...
	if (fb_event && fb_event->data && core_data) {
		if (event == MSM_DRM_SWITCH_EARLY_EVENT_BLANK) {
			...
			if (FRONT_PANEL_MODE == nubia_tp_mode) {
				goodix_ts_suspend(core_data);
				nubia_tp_mode = 0;
			}
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	r = goodix_ts_back_gpio_power_request(core_data,true);
	if (r < 0)
		goto out;
	r = goodix_back_ts_power_on(core_data);
	if (r < 0)
		goto out;
	r = goodix_ts_back_gpio_power_on_off(core_data,true);
	if (r)
		goto out;
