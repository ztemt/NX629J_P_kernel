		...
		char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "%d,%d\n",
			atomic_read(&gsx_gesture->registered), b_gesture_enabled);
#endif
}
		...
		const char *buf, size_t count)
{
	...
	ts_err(" back Tmp value =%d", tmp);

	if (tmp == 1) {
		if (atomic_read(&gsx_gesture->registered)) {
			ts_debug("back Gesture module has aready registered");
			return count;
		}
		...
		if (!atomic_read(&gsx_gesture->registered)) {
			ts_debug("back Gesture module has aready unregistered");
			return count;
		}
		ts_debug("back Start unregistered gesture module");
...
#endif
			atomic_set(&gsx_gesture->registered, 0);
			ts_info("back Gesture module unregistered success");
		} else {
#ifdef GOODIX_TS_BACK_GESTURE
			b_gesture_enabled = 1;
#endif
